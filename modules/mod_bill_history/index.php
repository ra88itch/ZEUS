...
function getInvoice($date){
	$invoice = array();
	if($date == 0){
		$sql = "SELECT * FROM `invoice` WHERE (DATE(`checkin`)=CURDATE() OR DATE(`checkin`)=subdate(current_date, 7)) AND `checkout` != '0000-00-00 00:00:00' ORDER BY `checkout` DESC";
	}else{
		$sql = "SELECT * FROM `invoice` WHERE (DATE(`checkin`)='$date' OR DATE(`checkin`)=subdate('$date', 7)) AND `checkout` != '0000-00-00 00:00:00' ORDER BY `checkout` DESC";
	}	
	//$sql = "SELECT * FROM `invoice` WHERE (DATE(`checkin`)>=subdate(current_date, 30)) AND `checkout` != '0000-00-00 00:00:00' ORDER BY `checkin` DESC";
	...
	while($results = mysql_fetch_assoc($query)) {
		...
		if($results['zone'] > 0){
			...
			if($results['zone_extra']==''){
				$detail = array('ลูกค้าเงินสด  '.$results['id'], '0');
			}else{
...
function chkBillDetails($invoiceID, $memberID, $print, $cash, $change, $invoiceBill){
	...
	while($results = mysql_fetch_assoc($order_query)) {
		...
		if($menuDetail[1] == '8' || $menuDetail[1] == '9'){
	...
	if(empty($details)) {
		...
		if($chkMember != '0'){
			$member_status = true;
			$total = $massage_total+$snooker_total+$sauna_total+$food_total+$member_total+$drink_total; // true
...
switch($type){
	case 'getInvoice':
		$date =  mysql_real_escape_string($_POST['date_val']);
		getInvoice($date);
		break;
	case 'chkBillDetails':
