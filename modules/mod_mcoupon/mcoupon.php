...
function getCouponName($coupon_ref){
	$sql = "SELECT `coupon_name` FROM `coupon` WHERE `id`='$coupon_ref'";
	$query = mysql_query($sql);
	$result = mysql_fetch_assoc($query);
...
<section id="sec-account">
	<div class="warp">
		<?php showNav('coupon', $_GET['mod'], $_GET['page']);?>		
		<div class="onerow">
			<div class="result">
				<div class="col12">
					<p>
						<table>
							...
							$sql = "SELECT * FROM `coupon` LIMIT ".$start_on.", 20";
							$query = mysql_query($sql);
				...
				<div id="add" class="add">ADD NEW COUPON</div>
			</div>
		</div>
	</div>
</section>