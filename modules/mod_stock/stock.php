...
function rowsCount($table){
	$sql = "SELECT COUNT(*) FROM $table WHERE `store_stock`='1'";
...
<section id="sec-stock">
	<div class="warp">
		...
		if(isset($_GET['id']) && $_GET['id']!=''){
			$sql = "SELECT * FROM `stock` WHERE `id`='".$_GET['id']."' AND `store_stock`='1' ORDER BY `code` ASC, `name` ASC";
			$query = mysql_query($sql);
				...
				<div class="col12">
					<p>
						<table>
							...
							$sql = "SELECT * FROM `stock` WHERE `store_stock`='1' ORDER BY `code` ASC, `name` ASC";
							$query = mysql_query($sql);
