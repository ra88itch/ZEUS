...
function chkOrder($array){
	if($array!=''){
		$i = 1;
		$html = '<tr>';
		$sql = "SELECT SUM(`order`.`unit`) AS `unit`, `order`.`menu_id` AS `menu_id` FROM `order` JOIN `menu` ON `menu`.`id`=`order`.`menu_id` WHERE `order`.`order_inv` IN ($array) AND `order`.`order_status`='5' GROUP BY `order`.`menu_id` ORDER BY `menu`.`type_by_cooking` ASC";
		$query = mysql_query($sql);
	...
	//$sql = "SELECT SUM(`order`.`unit`) AS `unit`, `order`.`menu_id` FROM `order` JOIN `menu` ON `menu`.`id`=`order`.`menu_id` WHERE `order`.`order_inv` IN ($array) AND `order`.`order_status`='5' ORDER BY `menu`.`type_by_cooking` ASC GROUP BY `menu_id` ";
	$sql = "SELECT SUM(`order`.`unit`) AS `unit`, `order`.`menu_id` AS `menu_id` FROM `order` JOIN `menu` ON `menu`.`id`=`order`.`menu_id` WHERE `order`.`order_inv` IN ($array) AND `order`.`order_status`='5' GROUP BY `order`.`menu_id` ORDER BY `menu`.`type_by_cooking` ASC";
	//return $sql;
	$query = mysql_query($sql);
	while($result = mysql_fetch_assoc($query)){
...
while($result = mysql_fetch_assoc($query)){
	if($result['gt']>=0){
		...
		$chkPayCashPercent = $chkPayCash*5/100;
		$grand_pay_cash .= ' / ค่าบริการ '.$chkPayCashPercent.' บาท';
		$grand_after_pay_cash = 'เงินสดคงเหลือ '. $after_pay_cash.' บาท';
		$grand_discount = 'ส่วนลดค่าอาหาร '.$discount.' บาท';


		$html = '<h1  style="font-size:60px">สรุปรายรับวันที่ '.$date.' </h1>';
		$html .= '<table>';
		$html .= '<tr>';
		$html .= '<td width="50%">';
		...
		$html .= '<tr>';
		$html .= '<td width="50%" align="center">';
		$html .= '<b>บริการเช่าตู้ล๊อกเกอร์</b>';
		$html .= '</td>';
		$html .= '<td width="50%">';
		...
		$html .= '<tr>';
		$html .= '<td width="50%">';
		$html .= chkLocker($array);
		$html .= '</td>';
		$html .= '<td width="50%">';
		$html .= chkDiscount($array);
		$html .= chkeDiscount($array);
		$html .= '</td>';
		$html .= '</tr>';
		$html .= '</table>';
...
$total = 0;
$total_html = 0;
$html = '<h1  style="font-size:60px">ตารางการทำงานพนักงานนวด วันที่  '.$date.' </h1>';
$html .= '<table border="1"><tr><td width="15%">หมายเลข</td><td width="35%">ประเภทบริการ</td><td width="35%">เวลา</td><td width="15%">เบิกจ่ายพนักงาน</td></tr>';
$sql = "SELECT (`times_in_min`/60) AS `hours`, `massage_type`, `employee_id`, `end` FROM `order_massage` WHERE DATE(`end`)='$date' AND `times_in_min`!='0' AND `massage_type`<'4' ORDER BY `start` ASC";
$query = mysql_query($sql);
while($result = mysql_fetch_assoc($query)){
	...
	//$total_html = $total_html+($result['hours']*$charge_per_hour);
	$html .= '<tr><td width="15%">'.$result['employee_id'].'</td><td width="35%">'.$chkMassage.'</td><td width="35%">'.$result['end'].'</td><td width="15%">'.($result['hours']*$charge_per_hour).'</td></tr>';
}
//$html .= '<tr><td width="85%" align="right">รวม</td><td width="15%">'.$total_html.'</td></tr>';
...
$sql = "SELECT SUM(`grand_total`) AS `gt`, `employee_id` AS `employee_id`, `inv_ref` FROM `invoice_bill` WHERE DATE(`checkout`)='$date' GROUP BY `employee_id`";
$query = mysql_query($sql);
while($result = mysql_fetch_assoc($query)){
	if($result['gt']>=0){
		$pdf->AddPage();
		...
		$array = '';
		$i = 0;
		$sql_2 = "SELECT `inv_ref`, `grand_total`, `payment`, `discount` FROM `invoice_bill` WHERE DATE(`checkout`)='$date' AND `employee_id`='".$result['employee_id']."'";
		$query_2 = mysql_query($sql_2);
		while($result_2 = mysql_fetch_assoc($query_2)){
			if($result_2['grand_total']>=0){
				if($result_2['payment'] == 1){
		...
		$html = '<h1 style="font-size:60px">สรุปรายรับวันที่ '.$date.' ของ '.accountName($result['employee_id']).'</h1>';
		$html .= '<table>';
		$html .= '<tr>';
		$html .= '<td width="50%">';
		$html .= '<div><b>'.$grand_total.'</b></div><div>'.$grand_cash.'</div><div>'.$grand_credit.'</div><div>'.$grand_entertain.'</div>';
...
*/
		...
		$html .= '<b>รายการอาหาร-เครื่องดื่ม</b>';
		$html .= '</td>';
		$html .= '</tr>';
		$html .= chkOrder($array);
		$html .= '</table>';
...
$pdf->Output('dailyreport-'.$date.'.pdf', 'I');
?>