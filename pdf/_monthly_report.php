...
function chkOrder($array){
	//return '<tr><td style="width:50%;">'.$array.'</td><td style="width:50%;"></td></tr>';
	if($array!=''){
		$i = 1;
		$html = '<tr>';
...
function chkRestaurant($array){
	$sql = "SELECT COUNT(*) FROM `order_restaurant` WHERE `order_inv` IN ($array) AND `order_status`='5'";
	$query = mysql_query($sql);
	$result = mysql_fetch_assoc($query);
	return $result['COUNT(*)'];
}
...
$total_html2 = $total_html;
$total_html = 0;
$html = '<h1  style="font-size:60px">ตัดบัตรเครดิตรายวัน 24.00 น.</h1>';
$html .= '<table border="1"><tr><td width="50%">วันที่</td><td width="50%">รายรับบัตรเครดิต</td></tr>';
$sql = "SELECT SUM(`discount`) AS `discount`, DATE(`checkout`) AS `txt_date`, `grand_total` FROM `invoice_bill` WHERE DATE_FORMAT(`realtimes`, '%Y-%m')='$date' GROUP BY DATE(`checkout`)";
$query = mysql_query($sql);
while($result = mysql_fetch_assoc($query)){
	if($result['grand_total']>=0){
		$credit = getSum($result['txt_date'],2);
		$html .= '<tr><td width="50%">'.$result['txt_date'].'</td><td width="50%" align="right">'.$credit.'</td></tr>';
	}
}
$html .= '</table>';
$htmlcontent=stripslashes($html);
...
$array = '';
$i = 0;
$html = '<h1  style="font-size:60px">รายการยอดขาย วันที่  '.$date.' </h1>';
$sql = "SELECT `grand_total`, `inv_ref` FROM `invoice_bill` 
INNER JOIN `invoice` ON `invoice_bill`.`inv_ref`=`invoice`.`id`
WHERE DATE_FORMAT(`invoice_bill`.`checkout`, '%Y-%m')='$date'";
$query = mysql_query($sql);
