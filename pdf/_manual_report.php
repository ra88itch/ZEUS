...
function chkOrder($array){
	...
	if($array!=''){
		...
		$sql = "SELECT SUM(`order`.`unit`) AS `unit`, `order`.`menu_id` AS `menu_id` FROM `order` JOIN `menu` ON `menu`.`id`=`order`.`menu_id` WHERE `order`.`order_inv` IN ($array) AND `order`.`order_status`='5' AND `menu`.`type_by_cooking`='9' GROUP BY `order`.`menu_id` ORDER BY `menu`.`type_by_cooking` ASC";
		$query = mysql_query($sql);
		while($result = mysql_fetch_assoc($query)){
...
$array = '';
$i = 0;
$html = '<h1  style="font-size:60px">รายการยอดขาย ปี 2017-Now </h1>';
$sql = "SELECT `grand_total`, `inv_ref` FROM `invoice_bill` 
INNER JOIN `invoice` ON `invoice_bill`.`inv_ref`=`invoice`.`id`
WHERE DATE_FORMAT(`invoice_bill`.`checkout`, '%Y')='2017' OR DATE_FORMAT(`invoice_bill`.`checkout`, '%Y')='2018'";
$query = mysql_query($sql);
